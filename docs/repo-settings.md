...
checks, local-only App Store export-policy checks, plist and entitlement
validation, diff checks, Xcode project generation, Debug tests, and Release and
App Store builds. Hosted app tests also prove that the privacy manifest is
present in the built application bundle.
